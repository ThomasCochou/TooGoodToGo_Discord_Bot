...
class MyClient(discord.Client):
    ...
    async def check_new_basket(self, discord_client):
    ...
    async def send_new_basket(self,discord_client,new_msg):
        for msg in new_msg :
            await self.channel.send(msg)

    async def get_last_msg(self,discord_client):
        last_msg = list()
    ...
    async def clear(self, discord_client):
        async for msg in self.channel.history():
            await msg.delete()
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith('get stores'):
            response = self.tgtg_client.get_items()
...
class MyCog(commands.Cog):
    ...
    async def printer(self):
        self.index += 1
        last_msg = await self.discord_client.get_last_msg(self.discord_client)
        new_msg = await self.discord_client.check_new_basket(self.discord_client)

        if last_msg != new_msg :
            await self.discord_client.clear(self.discord_client)
            await self.discord_client.send_new_basket(self.discord_client,new_msg)
